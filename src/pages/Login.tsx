...
const Login = () => {
  ...
  useEffect(() => {
    const handleGoogleRedirect = async () => {
      try {
        ...
        const { checkRedirectResult } = await import('@/lib/firebase').catch((importError) => {
          console.error('Failed to import Firebase module:', importError);
          return { checkRedirectResult: async () => ({ success: false, user: null }) };
        });
  ...
  const handleGoogleLoginResult = async (googleUser: any) => {
    ...
    try {
      ...
      // Use API authentication instead of direct Supabase with error handling
      const { authAPI } = await import('@/lib/api').catch((importError) => {
        console.error('Failed to import API module:', importError);
        throw new Error('Unable to initialize authentication service');
      });
      ...
      <div className="min-h-screen bg-gray-100 flex items-center justify-center">
        <div className="bg-white p-8 rounded-lg shadow-lg max-w-md w-full">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">Unable to initialize authentication. Please refresh the page.</p>
            <button
